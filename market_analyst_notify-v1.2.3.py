"""
Level 1 AI Market Analyst -- v1.2.1 (formatting + per-symbol notifications)

Builds directly on v1.2.0's Phase 1 facts engine + Phase 2 LLM analyst --
no logic changes there. This version only changes:
  1. The ntfy message is shorter and better-spaced (dropped the long
     objective_analysis wall of text from the push -- it's still printed
     to the Actions log for auditing, just not crammed into the phone
     notification).
  2. Each symbol now sends its OWN notification instead of all symbols
     being concatenated into one giant message. This matters as soon as
     you add more than one SYMBOLS entry -- without this, adding ETHUSDT
     etc. would make the single notification progressively more
     congested. Each push's title includes the symbol + bias so you get
     the headline without even opening it.
"""
...
5. Length matters -- this is read as a phone push notification, not a report. Keep interpretation to 3-4 sentences (roughly 60-90 words), key_risk to a single sentence, and each scenario's path to one concise sentence. Prioritize the single most important thing to say over completeness; a shorter, sharper read beats a longer, hedged one.
6. Respond with ONLY a single JSON object -- no markdown fences, no preamble, no text outside the \
...
def format_analyst_report(analysis):
    """
    Formats ONLY the interpretation-forward part of the analysis for the
    push notification -- objective_analysis is deliberately left out here
    (it's still printed to the Actions log in main() for auditing) since
    restating every raw fact in a phone notification is what was making
    this feel congested. This keeps the push to the part a trader
    actually wants to read at a glance.
    """
    lines = [
        f"{bias_arrow(analysis['bias'])} {analysis['bias']}  ·  {analysis['trade_readiness']} readiness  ·  {analysis['confidence']} confidence",
        "",
        analysis["interpretation"],
        "",
        f"Key risk: {analysis['key_risk']}",
    ]
...
def send_ntfy(title, text):
    # HTTP headers must be Latin-1/ASCII -- unlike the message body (sent as
    # UTF-8 bytes), a stray character like an em dash or emoji in the title
    # would crash the request. Strip anything non-ASCII defensively so a
    # future formatting tweak can't silently break notifications again.
    safe_title = title.encode("ascii", errors="ignore").decode("ascii") or "Level 1 Market Analyst"

    # ntfy caps message bodies at 4096 bytes; beyond that it either
    # truncates mid-sentence or converts to a downloadable attachment
    # depending on the client. The system prompt now asks the LLM to be
    # concise enough to avoid this, but this is a hard backstop: if a
    # message ever runs long anyway, cut it cleanly at a sentence/word
    # boundary and label it, rather than letting ntfy silently chop it
    # off mid-word with no indication anything was cut.
    LIMIT = 3800  # bytes, leaving headroom under ntfy's 4096 cap
    encoded = text.encode("utf-8")
    if len(encoded) > LIMIT:
        truncated = encoded[:LIMIT].decode("utf-8", errors="ignore")
        cutoff = max(truncated.rfind(". "), truncated.rfind("\n"))
        if cutoff > LIMIT * 0.6:  # only trim to the boundary if it's not too far back
            truncated = truncated[:cutoff + 1]
        text = truncated.rstrip() + "\n\n[...truncated, see Actions log for full analysis]"

    url = f"https://ntfy.sh/{NTFY_TOPIC}"
    requests.post(
        url,
        data=text.encode("utf-8"),
        headers={"Title": safe_title, "Priority": "default", "Tags": "chart_with_upwards_trend"},
        timeout=15,
    )
...
def main():
    for sym in SYMBOLS:
        ...
        try:
            ...
            title = f"{sym} - {analysis['bias']}"
            body = "Report only -- no trades placed.\n\n" + format_analyst_report(analysis)

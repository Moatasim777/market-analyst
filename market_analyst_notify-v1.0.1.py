...
Version: v1.0.1 (switched data source from Bybit to Kraken -- see notes below)

What this does:
  - Pulls 15m/1h/4h/1d candles for a list of symbols from Bybit's public API
...
Why Kraken and not Binance (or Bybit) here:
  Binance blocks requests coming from cloud/datacenter IPs (AWS, Azure,
  GitHub Actions, etc.) with an HTTP 451 error, regardless of where you
  personally are -- this is separate from anything to do with Pakistan.
  Bybit turned out to have the same kind of cloud-IP block (confirmed by
  developers hitting 403s from AWS/GitHub Actions specifically), so it
  isn't a safe substitute either. Kraken's public market-data endpoints
  don't have this reported issue, which is why the notifier uses Kraken.
  Your phone's browser dashboard stays on Binance since that already
  works fine from a normal mobile/residential connection.

  Kraken naming note: Kraken calls Bitcoin "XBT" instead of "BTC" for
  legacy reasons, so BTCUSDT is mapped to XBTUSDT internally.
...
# Kraken interval is in minutes; these line up exactly with our timeframes.
...
# Kraken uses "XBT" instead of "BTC". Extend this map if you add other
# symbols that Kraken names differently (e.g. it doesn't list BNB at all).
...
def fetch_klines(symbol, interval, limit=100):
    ...
    # result has one key named after the pair (naming can vary slightly) plus "last"
